  ...
  loadCache("strutture").then((response) => {
    ...
    if (nome.value !== "" && indirizzo.value !== "" && descrizione.value !== "") {
      ...
      callRemote(url, (result) => {
        ...
        strutture.push({ "nome": nome.value, "indirizzo": indirizzo.value, "descrizione": descrizione.value, "lonlat": point });
        saveCache(strutture, "strutture");
...
const render = () => {
  loadCache("strutture").then(strutture => {
    //   action();
    if (strutture) {
      ...
      strutture.forEach((element) => {
        ...
        htmlTab += temp_table.replace("%NAME", element.nome).replace("%ADDRESS", element.indirizzo).replace("%DESCRIPTION", element.descrizione);
      })
